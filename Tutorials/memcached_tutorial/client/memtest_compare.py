...
counter = 0
error_counter = 0
skip_till_the_end_of_packet = False
...
for iLine in inputFile:
	...
	if (iLineSplit[3] == "2"):
		#evalState(iLine)
		if skip_till_the_end_of_packet:
			skip_till_the_end_of_packet = False
			gLine = "0000000000000000 00 1 2";
		else:
			gLine = goldenFile.readline()
		if (compareLines(gLine, iLine) == False):
			if (iLineSplit[1] == "01" and gLine[14:15] ==  iLine[14:15]):
				continue
			if (iLineSplit[1] == "03" and gLine[12:15] ==  iLine[12:15]):
				continue
			if (iLineSplit[1] == "07" and gLine[10:15] ==  iLine[10:15]):
				continue
			if (iLineSplit[1] == "0F" and gLine[8:15] ==  iLine[8:15]):
				continue;
			if (iLineSplit[1] == "1F" and gLine[6:15] ==  iLine[6:15]):
				continue
			if (iLineSplit[1] == "3F" and gLine[4:15] ==  iLine[4:15]):
				continue
			if (iLineSplit[1] == "7F" and gLine[2:15] ==  iLine[2:15]):
				continue
			if (iLineSplit[1] == "FF" and gLine[0:15] ==  iLine[0:15]):
				print("Extra Zero bytes at Output line " + str(counter))
				skip_till_the_end_of_packet = True
				continue
			errorCheck(gLine, iLine)
			print("Error! Output line " + str(counter) + " not identical. \ng=" + gLine + "i=" + iLine)
			error_counter = error_counter + 1
			gLineSplit = gLine.split()
			if (gLineSplit[2] != iLineSplit[2]):
				print("Aborting: Unqeual packet length at line " + str(counter))
				print("Number of mismatch lines = " + str(error_counter))
				sys.exit(1)
	else:
		continue
	if (EOP == True):
