...
def parse_partial_rdbk_file(rdbk_file, start_address, bram_rdbk_file=0, bram_start_address=0, reverse=False, fast=False):
	...
	# fast means frame_data is kept in ASCII format (not converted to integers)
	if fast:
		frame_data = ['00000000000000000000000000000000']*(full_frame_data_index*FRAME_LENGTH)
	else:
		frame_data = [0]*(full_frame_data_index*FRAME_LENGTH)

	if reverse:
		parse_rdbk_file_reverse(rdbk_file, frame_data, fast)
	else:
	...
	if bram_rdbk_file:
		...
		# fast means frame_data is kept in ASCII format (not converted to integers)
		if fast:
			frame_data.extend(['00000000000000000000000000000000'] * (CLB_FRAMES * FRAME_LENGTH - len(frame_data) + full_frame_data_index * FRAME_LENGTH))
		else:
			frame_data.extend([0] * (CLB_FRAMES * FRAME_LENGTH - len(frame_data) + full_frame_data_index * FRAME_LENGTH))
		
		if reverse:
			parse_rdbk_file_reverse(bram_rdbk_file, frame_data, fast)
		else:
			parse_rdbk_file(bram_rdbk_file, frame_data, fast)

	return frame_data

def parse_rdbk_file_reverse(rdbk_file, frame_data, fast=False):
...
def parse_rdbk_file(rdbk_file, frame_data, fast=False):
	# fast means frame_data is kept in ASCII format (not converted to integers)
	if fast:
