...
if len(sys.argv) == 3:
	...
	rl_file_name = 0
	bitfile_name = sys.argv[2]
elif len(sys.argv) == 4:
	ll_file_name = sys.argv[1]
	rl_file_name = sys.argv[2]
	bit_file_name = sys.argv[3]
else:
	print("Expects at least two arguments: logic location file, [ram location file] and bit file")
	exit()
...
with open("hw_state.dump", 'w') as output_file:
	...
	for i in range(len(ram_name)):
		...
		value = get_lram_value_from_frame_data(x, y, 64, bit_frame_data, lram_bit_offset, lram_frame_address, lram_frame_offset, lram_xy, lram_bit)
